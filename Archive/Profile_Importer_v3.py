...
from dataclasses import field, fields, astuple, dataclass, InitVar, make_dataclass
...
def make_dc(data, params, name='Profile'):
    list_in = [(gf.nameify(k), type(v), v) for k, v in params.to_dict().items()]
...
class PixelConv:
    def __init__(
        ...
        **kwargs,
        ...
        if path is not None:
            ...
            self.outpath = (
                "C:\\Users\\j2cle\\Work Docs\\Data\\Analysis\\SIMS\\RICE\\220425 Man\\Results"
            )
        ...
        self.data_ref_intens = [key for key in self.data_ref.variables.mapping.keys() if params.Ion.lower() in key and "intens" in key][0]
        # self.data_ref_conc = [key for key in self.data_ref.variables.mapping.keys() if params.Ion.lower() in key and "conc" in key]

        if not np.isnan(params['SF']):
            self.sf = params['SF']
        elif self.data_ref_conc is not None:
            self.sf = (self.data_ref[self.data_ref_conc] / self.data_ref[self.data_ref_intens]).mean()
        else:
            self.sf = 5e14
    ...
    def data_2d(self):
        ...
        _data_2d["surf"] = self._matrix_ds.intens.sel(
            z=slice(None, self.surf_lyr, None)
        ).sum(["z"])
        _data_2d["back"] = self._matrix_ds.intens.sel(
            z=slice(self.back_lyr, None, None)
        ).sum(["z"])
        _data_2d["sliced"] = self._matrix_ds.intens.sel(
    ...
    def save(self):
        gf.save(self.res_dict, self.outpath, f"{self.sample}_{self.params.Ion}_info")
        ...
        gf.save(maps, self.outpath, f"{self.sample}_{self.params.Ion}_maps")
        ...
        gf.save(maps1, self.outpath, f"{self.sample}_{self.params.Ion}_mapsv1")
        ...
        if self.corr:
            ...
            gf.save(maps2, self.outpath, f"{self.sample}_{self.params.Ion}_mapsv2")
    ...
    def plots(
        ...
        if grp_plot is not None:
            ...
            if self.save_res:
                plt.savefig(f'{self.outpath}/{self.sample}_{self.params.Ion}_grp.png')
                plt.close()
        if map_plot is not None:
            ...
            if self.save_res:
                plt.savefig(f'{self.outpath}/{self.sample}_{self.params.Ion}_map.png')
                plt.close()
        ...
        if surf_plt is not None:
            ...
            plt.tight_layout()
            if self.save_res:
                plt.savefig(f'{self.outpath}/{self.sample}_{self.params.Ion}_surf.png')
                plt.close()
        ...
        if prof_plt is not None:
            ...
            plt.tight_layout()
            if self.save_res:
                plt.savefig(f'{self.outpath}/{self.sample}_{self.params.Ion}_prof.png')
                plt.close()
...
class BulkImport:
    def __init__(self, call, calc=False, folder="Bulk_import", **kwargs):
        self.calc = calc
        self.jar = gf.PickleJar(folder=folder, **kwargs)
        ...
        for sample in self.df_log.index:
            ...
            if not calc:
                if self.df_log["Import Info"][sample] == 'rice_raw':
                    ...
                    raw = gf.slugify(f'{samp}-raw_{meas}')
                    corr = gf.slugify(f'{samp}-corr_{meas}')
    ...
    def to_obj(self, **kwargs):
        ...
        for sample in self.df_log.index:
            ...
            if not calc and self.df_log["Import Info"][sample] == 'rice_raw':
                objs = db[db.str.contains(f"{gf.slugify(sample)}_obj")]
                if len(objs) == 0:
                    calc = True
                else:
                    self.objs = objs
            if calc and self.df_log["Import Info"][sample] == 'rice_raw':
                ...
                self.objs = f"{gf.slugify(sample)}_obj"
        return
    ...
    def datas(self, value):
        """Calculate constant, may shift to use the depth range instead."""
        self._datas = pd.Series(pd.concat([self.datas, pd.Series(value)],ignore_index=True).unique())
...
# #%%
# # test = BulkImport(["E-80", "E-Pixel"])
# test = BulkImport(["A-Na+", "E-Pixel"])
# test.to_obj(devs=np.nan,num_bins=5,save=True)
# obj1=test.objs['E-80-A-Na+']
# # obj2=test.objs['E-60-A-Na_2+']

# #%%
# obj1.calculate(surf_lyr = 3)
# #%%
# obj1.plots(map_plot=0,plt_type="raw_conc")
# obj1.plots(prof_plt=0,plt_type="raw_conc")

# # test.to_data(devs=np.nan,num_bins=5)
# # obj = {}
# # for key, val in test.datas.items():
# #     if "conc" in key:
# #         params = [x for x in test.params.keys() if "Na+" in x or "Na_2+" in x][0]
# #     else:
# #         params = key
