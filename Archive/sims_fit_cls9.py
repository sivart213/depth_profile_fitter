...
def c_np(depth, diff, conc, thick, temp, e_app, time, log_form=False):
    ...
    if log_form:
        ...
        return (conc / (2 * term_B)) * (
            erfc((depth - mob * e_app * time) / (2 * np.sqrt(diff * time)))
            + erfc(
                -(depth - 2 * thick + mob * e_app * time) / (2 * np.sqrt(diff * time))
            )
        )
...
def pivot_cleaner(table_in):
    ...
    table2 = (table1.pivot_table(index=[table1.columns[0], table1.columns[1]])
        .to_xarray()
    )
    ...
    # except ValueError:
    #     table_out = pd.DataFrame(table_out,
    #                              index=table_in.columns).T
    #     table_out = table_out.iloc[:len(index_labels),:]
    #     table_out.index = index_labels[::-1]
...
class Stats:
    ...
    def mean_abs_perc_err(self):
        ...
        return metrics.mean_absolute_percentage_error(
            self.meas, self.pred, **self.kwargs
        )
