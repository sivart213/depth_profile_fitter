...
def pivot_cleaner(table_in):
    ...
    series_count = table_in[::-1].count(axis=1)
    index_labels = []

    expected = list(np.arange(1,min(table_in.shape)+1, dtype=int))
    try:
        m = 0
        for n in range(len(expected)):
            # loc = series_count.index[m]
            locs = [series_count.index[m]]
            val = series_count.iloc[m]
            count = 1
            while val < expected[n]:
                m += 1
                val += series_count.iloc[m]
                locs.append(series_count.index[m])
            index_labels.append(series_count[locs].idxmax())
            m += 1
    except IndexError:
        index_labels = list(np.linspace(1,max(table_in.shape)+1, min(table_in.shape), dtype=int))[::-1]
...
class Analysis:
    ...
    def peaks_solver(self, num_of_peaks=2, stop_max=2.5, start_max=2, start_min=2, overlap=True):
        """Return sum of squared errors (pred vs actual)."""
        ...
        for x in range(len(self.start_topo)-1):
            if self.start_topo.iloc[x, 3] < 0:
                outer_int = x
                inner_int = x+1
                while inner_int < len(self.start_topo) and self.start_topo.iloc[inner_int, 3] < 0:
                    self.start_topo.iloc[outer_int, 3] += self.start_topo.iloc[inner_int, 3]
                    self.start_topo.iloc[inner_int, 3] = 0
                    inner_int += 1
        ave_peak = int(self.start_topo['comb_res'][self.start_topo['comb_res']<0].median())

        min_peaks = self.start_topo['comb_res'][(self.start_topo['comb_res'] <= ave_peak)]

        self.peak_df = pd.DataFrame(columns=['start_index', 'count', 'min_bound', 'max_bound', 'best_range'])
        ...
        self.peak_df['best_range'] = [limited_df.loc[limited_df['change']
                                                     [limited_df['start_index'] == x]
                                                     [::-1].idxmin(), 'index_range']
                                      for x in self.peak_df['start_index']]
        ...
        self.region_dict = {}
        self.region_dict['all'] = []

        for n in range(min(num_of_peaks,len(min_peaks))):
            self.region_dict[n] = self.peak_dict[n] + self.range_dict[n]
            self.region_dict['all'] += self.region_dict[n]

        return self.region_dict, self.peak_dict, self.start_topo, self.peak_df

    def peaks(self, peak=0, min_start=None, max_start=None, min_range=3, peak_range=None, max_range=None, pair_set=None, old_range=False,
