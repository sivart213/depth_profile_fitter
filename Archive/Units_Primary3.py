# -*- coding: utf-8 -*-
"""
Created on Mon Apr 11 15:21:51 2022
...
units_dict = {}
units_dict['length'] = {'m': 1, 'Ang': 1e-10, 'inch': 0.0254, 'ft': 0.3048,
                        'yd': 0.9144, 'mi': 1609.34, 'NM': 1852, 'ftm': 1.8288}
units_dict['mass'] = {'g': 1, 'lb': 453.59, 'oz': 48.35, 'ton':  0.907e6,
                      'ton_uk': 1.016e6, 't': 1e6, 'Mt': 1e12, 'Gt':  1e15}
units_dict['time'] = {'s': 1, 'mn': 60, 'hr': 3600, 'd':  86400, 'wk': 604800,
                      'mo': 2.628e+6, 'yr': 3.154e+7, 'dec': 3.154e+8, 'c': 3.154e+9}
units_dict['temp'] = {'K': 1}
units_dict['curr'] = {'A': 1}
...
si_list = {'length': 'm', 'mass': 'g', 'time': 's', 'temp': 'K', 'curr': 'A',
           'mole': 'mol', 'energy': 'J', 'volt': 'V'}

units_df = pd.DataFrame([(m, n, o) for m in list(units_dict.keys())
                         for n, o in units_dict[m].items()], columns=['Type', 'unit', 'value'])
...
class Search:
    ...
    def __init__(self, unit='', base_unit='', user_list=None, user_dict={}):
        ...
        _pre_base = {'T': 1e12, 'G': 1e9, 'M': 1e6, 'k': 1e3, 'd': 1e-1, 'c': 1e-2,
                     'm': 1e-3, 'u': 1e-6, 'n': 1e-9, 'p': 1e-12, 'f': 1e-15}
        try:
            _pre_base = {x: _pre_base[x] for x in user_list}
        except KeyError:
            _pre_base = {x: _pre_base[x] for x in
                         ['T', 'G', 'M', 'k', 'd', 'c', 'm', 'u', 'n', 'p', 'f']}
...
class BaseUnits(object):
    ...
    def __add__(self, other):
        ...
        if isinstance(other, BaseUnits):
            if other.unit == self.unit:
                ...
                other = 0
        return self.__class__(self.value+other,self.unit,self.exp,self.print_unit,self.base_unit,self._fig_set).set_units(self.class_units)
    ...
    def __rsub__(self, other):
        ...
        if isinstance(other, BaseUnits):
            if other.unit == self.unit:
                ...
                other = 0
        return self.__class__(other-self.value,self.unit,self.exp,self.print_unit,self.base_unit,self._fig_set).set_units(self.class_units)
    ...
    def __mul__(self, other):
        ...
        if isinstance(other, BaseUnits):
            ...
            return self.__class__(self._value*other._value, unit, exp, unit, unit)
        else:
            return self.__class__(self.value*other,self.unit,self.exp,self.print_unit,self.base_unit,self._fig_set)
    ...
    def __rtruediv__(self, other):
        ...
        if isinstance(other, BaseUnits):
            ...
            units2 = other.parcer(other.base_unit)
            unit, exp = self.joiner(units1, units2)
            return self.__class__(other._value/self._value, unit, exp, unit, unit)
        else:
            return self.__class__(other/self.value,self.unit,self.exp,self.print_unit,self.base_unit,self._fig_set)
    ...
    def figs(self, num):
        ...
        if isinstance(num, np.ndarray):
            for m, n in enumerate(num):
                num[m] = round(n, -(int("{:e}".format(n).split("e")[1]) - (self._fig_set-1)))
        else:
            num = round(num, -(int("{:e}".format(num).split("e")[1]) - (self._fig_set-1)))
        return num
