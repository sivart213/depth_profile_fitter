...
class Analysis:
    ...
    def peaks_solver(self, num_of_peaks=2, stop_max=2.5, start_max=2, start_min=2, overlap=True):
        ...
        self.limited_error = limited_df.pivot_table(index='index_range', columns='start_index', values='error')

        start_min_df = limited_df.pivot_table(index='index_range', columns='start_index', values='min')
        start_max_df = limited_df.pivot_table(index='index_range', columns='start_index', values='max')
    ...
    def peaks_alt(self, peak=0, start=None, stop=None, full_stop=None, min_range=3, pair_set=None,
        ...
        if start is None:
            start = self.peak_df.iloc[peak, 2]
        if stop is None:
            stop = self.peak_df.iloc[peak, 3]
        if full_stop is None:
            full_stop = np.array(self.region_dict['all'])[:,1].max() + start
        ...
        pairs_new = [x for x in pairs if x[0] >= start and
                     x[0] <= stop and x[0] + x[1] <= full_stop]

        focii = self.focus(pairs=pairs_new, pair_names=pair_names)
        focii = (focii[(focii['diff'] != 1e-11) &
                       (focii.index.get_level_values(0) < stop) & # makes  sure that start is under desired stop (stay in range)
    ...
    def peaks(self, peak=0, min_start=None, max_start=None, min_range=3, peak_range=None, max_range=None, pair_set=None, old_range=False,
        ...
        ranges[ranges <= 0] = 1
        ranges[ranges < 1] = 0
        while (ranges[::-1].idxmax().idxmin() <= self.peak_df.iloc[peak, 0]
               and ranges[::-1].idxmax().min()+1 <=
               ranges[self.peak_df.iloc[peak, 0]].dropna().index.max()):
            ranges.loc[ranges[::-1].idxmax().min()+1, ranges[::-1].idxmax().idxmin()] = 1
