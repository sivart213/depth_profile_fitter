...
class Matrix_Ops:
    ...
    def fit_curves(self):
        ...
        generic discription
        """
        col = ['Location', 'error', 'D', 'C0', 'x0 index', 'x1 index',
        ...
        if hasattr(self, 'min_array'):
            return pd.DataFrame([[tuple(x),
                                ...
                                self.obj_matrix.iloc[tuple(x)].D,
                                self.obj_matrix.iloc[tuple(x)].C0,
                                ...
                                self.obj_matrix.iloc[tuple(x)].limited_depth,
                                self.obj_matrix.iloc[tuple(x)].limited_conc,
                                self.obj_matrix.iloc[tuple(x)].data['Na'].to_numpy()]
...
class Analysis:
    ...
    def __init__(self, obj_matrix, **kwargs):
        ...
        generic discription  cols = [15,21], rows = [-17,-11]
        """
        if isinstance(obj_matrix,Matrix_Ops):
            self.obj = obj_matrix
            obj_matrix =  self.obj.obj_matrix

        self.obj_matrix = obj_matrix

    def plot(self, info='error', name=None, matrix=None, **kwargs):
        """
        Calculate.
        ...
        """
        if matrix is None:
            to_plot = self.get_matrix(info)
        else:
            to_plot = matrix

        if self.obj_matrix.iloc[0,0].__class__.__name__ == 'Fit_Profile':
            if name is None:
                name = 'Fit Profile'
        ...
        if self.obj_matrix.iloc[0,0].__class__.__name__ == 'Pred_Profile':
            if name is None:
                name = 'Pred Profile'
    ...
    def get_matrix(self, attr='x0_index'):
        """
        Calculate.
    ...
    def check_error(self, **kwargs):
        ...
        generic discription
        """
        self.error_matrix = self.obj_matrix.applymap(lambda x: Error(x.data, x.pred,x.x0_index, x.x1_index) if not isinstance(x, int) else 1)
        # self.error_operator = Composite()
        # self.error_matrix.applymap(lambda x: self.error_operator.add(x)
        #                               if isinstance(x, int) else None)

        return self.error_matrix.applymap(lambda x: x.err(**kwargs)  if not isinstance(x, int) else 1)
    ...
    def residuals(self,loc, log_form=False, plot=True):
        ...
        if log_form:
            self.resids = np.log10(self.data['Na'].to_numpy()) - np.log10(self.pred)
            if plot:
                sns.residplot(x=np.log10(self.pred), y=np.log10(self.data['Na'].to_numpy()), lowess=True, color="g")

        else:
            self.resids = self.data['Na'].to_numpy() - self.pred
            if plot:
                sns.residplot(x=self.pred, y=self.data['Na'].to_numpy(), lowess=True, color="g")
...
by_D = Matrix_Ops(sample_data, 'Pred_Profile', 20)
# %% run pred matrix analysis
by_D.set_bkg(to_log=False, instr='base', use_sample_w=False)
# error(instr='logic' or 'base' or 'none', use_sample_w=True or false)
# by_D.error_calc(run_bkg=True, reset=False, get_best=True, to_log=True,
#                 instr='logic', use_sample_w=True)

by_D.find_ranges(method='fst_fwd', get_best=True, to_log=False,instr='base', use_sample_w=False)
