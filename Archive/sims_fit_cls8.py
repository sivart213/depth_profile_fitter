...
def peak_cycles(obj, focus_df,  min_start=None, max_start=None, min_range=2, peak_range=None, max_range=None, pair_set=None,old_range=False):
...
def pivot_cleaner(table_in):
    ...
    count_row = table_in[::-1].count(axis=1)

    if np.sum(count_row == table_out.shape[1]) > 2:
        min_loc = count_row.index.get_loc(count_row[count_row==table_in.shape[1]][::-1].idxmin())+1
        while min_loc <= len(count_row)-1:
            table_out.loc[count_row.index[min_loc],:] = table_out.loc[count_row.index[min_loc],:].interpolate(method='linear', limit=2)
            count_row = table_out[::-1].count(axis=1)
            min_loc = count_row.index.get_loc(count_row[count_row==table_in.shape[1]][::-1].idxmin())+1

    if not table_out.iloc[:,0].isna().any() and not table_out.iloc[0,:].isna().any():
        return table_out

    count_col = table_in[::-1].count(axis=0)

    index_labels = []

    expected = list(np.arange(1,count_col.iloc[0]+1, dtype=int))
    try:
        m = 0
        for n in range(len(expected)):
            # loc = count_row.index[m]
            locs = [count_row.index[m]]
            val = count_row.iloc[m]
            count = 1
            while val < expected[n]:
                m += 1
                val += count_row.iloc[m]
                locs.append(count_row.index[m])
            index_labels.append(count_row[locs].idxmax())
            m += 1
    except IndexError:
        index_labels = list(np.linspace(1,max(table_in.shape)+1, count_col.iloc[0], dtype=int))[::-1]

    table_out = [table_in.iloc[:,x].dropna().to_list() for x in range(len(table_in.columns))]
    table_out = pd.DataFrame(table_out,
                           index=table_in.columns,
                           columns=index_labels[::-1]).T
    return table_out
...
class DataProfile:
    ...
    def data_treatment(self):
        ...
        if self.params['Type'] == 'NREL MIMS':
            ...
            data_raw = pd.read_csv(self.params['Data File Location'], delimiter='\t',
                                   header=None, names=headers, index_col=False,
                                   skiprows=16).dropna().astype(float)
    ...
    def __init__(self, sims_obj, start_index=None, stop_index=None, **kwargs):
        ...
        if stop_index is not None:
            if isinstance(stop_index, (float, np.float)):
                index =  gf.find_nearest(self.depth, stop_index)
...
class ProfileOps(Component):
    ...
    def set_error(self, start=None, stop=None, save_res=True, instr='None',
        ...
        if (self.prof.std_values['diff'].isin([np.log10(self.prof.diff)]).any() or
                self.prof.std_values['conc'].isin([np.log10(self.prof.conc)]).any()):
            self.error = 1
        else:
            self.start = start
            self.stop = stop
...
class MatrixOps:
    ...
    def __init__(self, sims_obj, cls_type, xrange=[None, None, None, None],
        ...
        if 'fit' in cls_type.lower():
            ...
            [self.obj_operator.add(ProfileOps(
                ...
                for x in self.xrange for y in self.yrange if (x + range_lim <= y)]
    ...
    def xrange(self, value):
        ...
        if 'ind' in value[3].lower():
            self._xrange = np.linspace(value[1], min(value[2], self.max_ind), min(self.size, self.max_ind + 1), dtype=int)
        elif 'lin' in value[3].lower():
            self._xrange = np.linspace(value[1], min(value[2], self.max_depth), min(self.size, self.max_ind + 1))
    ...
    def yrange(self, value):
        ...
        if 'ind' in value[3].lower():
            self._yrange = np.linspace(value[1], min(value[2], self.max_ind), min(self.size, self.max_ind + 1), dtype=int)
        elif 'lin' in value[3].lower():
            self._yrange = np.linspace(value[1], min(value[2], self.max_depth), min(self.size, self.max_ind + 1))
        elif 'log' in value[3].lower():
...
class Analysis:
    ...
    def pks_generate(self, num_of_peaks=2, stop_max=2.5, start_max=2, start_min=2, range_min=1, overlap=True):
        """Return sum of squared errors (pred vs actual)."""
        ...
        self.limited_error = pivot_cleaner(limited_df.pivot_table(index='index_range', columns='start_index', values='error'))


        start_min_df = pivot_cleaner(limited_df.pivot_table(index='index_range', columns='start_index', values='min'))
        start_max_df = pivot_cleaner(limited_df.pivot_table(index='index_range', columns='start_index', values='max'))
        ...
        peak_df = pd.DataFrame(columns=['start_index', 'count', 'left_bound', 'right_bound', 'best_range'])
        ...
        no_chng = grp_all.agg({'change':'nunique','index_range':'idxmax'})[grp_all['change'].nunique()['change'] <= 1]

        limited_df['change'][no_chng['index_range']] = 0

        grp_mins = limited_df[limited_df['change'] == 0].groupby(['start_index'], as_index=False, sort=False)
        by_range = grp_mins[self.family_df.columns].tail(1)

        # save range at starts where change is min
        # peak_df['best_range'] = [limited_df.loc[limited_df['change']
        #                                              [limited_df['start_index'] == x]
        #                                              [::-1].idxmin(), 'index_range']
        #                               for x in peak_df['start_index']]
        peak_df['best_range'] = by_range['index_range'][by_range['start_index'].isin(peak_df['start_index'])].to_numpy()
        ...
        for n in range(len(min_peaks)):
            peak_dict[n] = list(limited_df[['start_index', 'index_range']]
            ...
            for i, x in enumerate(range_df['index_range']):
                ...
                if np.isnan(range_df.loc[i, 'start_max_%d' % n]).all():
                    range_df['start_max_%d' % n] = range_df['start_max_%d' % n
                                                            ].fillna(min_peaks.index[n])
        ...
        range_dict = {i: (list(range_df.loc[:, ('start_min_%d' % i, 'index_range')]
                               .itertuples(index=False, name=None)) +
        ...
        return ({'ridge pairs': self.region_dict, 'valley pairs': self.peak_dict},
                {'valley lengths': valley_leng, 'by_range': by_range, 'by_error': by_error},
                self.peak_df)

    def pks_analyze(self, peak=0, min_start=None, max_start=None, min_range=3, peak_range=None, max_range=None, pair_set=None, old_range=False,
              pair_names=['start_index', 'index_range'], **kwargs):
        ...
        peak_range = gf.find_nearest(self.limited_error.index.to_numpy(), peak_range, False)
        ...
        while (ranges[::-1].idxmax().idxmin() <= self.peak_df.iloc[peak, 0] and ranges[::-1].idxmax().min()+1 <= ranges[self.peak_df.iloc[peak, 0]].dropna().index.max()):
        ...
        # ridgeline[ridgeline.index <= self.peak_df.iloc[peak, 0]] = ridgeline[self.peak_df.iloc[peak, 0]]

        ridge = pd.DataFrame([[1 if (y <= ridgeline[x]
                                     and x <= ranges[::-1].idxmax().idxmin())
                               else 0 for x in ridgeline.index]
                              for y in ranges.index],
                             columns=ranges.columns, index=ranges.index)
        ...
        pairs_new = [x for x in pairs if x[0] >= min_start and
                     x[0] <= max_start and x[1] <= max_range]

        focii = self.focus(pairs=pairs_new, pair_names=pair_names)
        focii = (focii[(focii['diff'] != 1e-11) &
                       (focii.index.get_level_values(0) < max_start) & # makes  sure that start is under desired stop (stay in range)
                       (focii.index.get_level_values(1) > min_range) & # makes sure that range always long enough
                       (focii['stop_index'] > self.peak_df.iloc[peak, 0])]) # makes sure that stop never before the local min -> range always straddles local min
    ...
    def auto_focus(self, min_start=None, max_start=None, min_range=2, peak_range=None, max_range=None, pair_set=None, old_range=False):
    ...
    def focus_report(self, peak=0, df=None, focus_stats=None, focii=None):
        if not hasattr(self, 'auto_focus_df'):
            df = pd.DataFrame(columns=['count',
                                       ...
                                       'conc', 'conc std',
                                       'min index', 'min loc',
                                       'Start indices', 'Range indices', 'Stop indices',
                                       'Start locs', 'Range locs', 'Stop locs',
                                       ],
                              index=self.peak_df.index, dtype=float)
        ...
        # df.loc[peak, 'min index'] = focii.index.get_level_values(0).min()
        # df.loc[peak, 'stop index (max)'] = focii['stop_index'].max()
        df.loc[:, 'Start indices':'Stop locs'] = df.loc[:, 'Start indices':'Stop locs'].astype(object)
        df.at[peak, 'Start indices'] = [focii.index.get_level_values(0).min(),
                                        gf.find_nearest(self.depth, focus_stats.loc['mean', 'start_loc']),
                                        focii.index.get_level_values(0).max()]
        df.at[peak, 'Range indices'] = [focii.index.get_level_values(1).min(),
                                        self.peak_df.iloc[peak, 4],
                                        focii.index.get_level_values(1).max()]
        df.at[peak, 'Stop indices'] = [int(focus_stats.loc['min', 'stop_index']),
                                       int(focus_stats.loc['mean', 'stop_index']),
                                       int(focus_stats.loc['max', 'stop_index'])]

        df.at[peak, 'Start locs'] = [focus_stats.loc['min', 'start_loc'],
                                     focus_stats.loc['mean', 'start_loc'],
                                     focus_stats.loc['max', 'start_loc']]
        df.at[peak, 'Range locs'] = [self.depth[df.loc[peak, 'Range indices'][0]],
                                     self.depth[df.loc[peak, 'Range indices'][1]],
                                     self.depth[df.loc[peak, 'Range indices'][2]]]
        df.at[peak, 'Stop locs'] = [focus_stats.loc['min', 'stop_loc'],
...
class Plotter(Analysis):
    ...
    def map_plot(self, name=None, info=[None, None, None], matrix=None, conv=[1, 1],
        ...
        # if to_plot.shape[0] > to_plot.shape[1]:
        #     temp = [to_plot.iloc[:,x].dropna().to_list() for x in range(len(to_plot.columns))]
        #     to_plot = pd.DataFrame(temp,
        #                            index=to_plot.columns,
        #                            columns=np.linspace(to_plot.index.min(),to_plot.index.max(),to_plot.shape[1], dtype=int)).T
        # if to_plot.shape[0] < to_plot.shape[1]:
        #     temp = [to_plot.iloc[:,x].dropna().to_list() for x in range(len(to_plot.index))]
        #     to_plot = pd.DataFrame(temp,
        #                            index=to_plot.index,
        #                            columns=np.linspace(to_plot.columns.min(),to_plot.columns.max(),to_plot.shape[0], dtype=int)).T
