...
import numpy as np
import pandas as pd
import sims_fit_cls9 as sfc
import General_functions as gf
import Units_Primary3 as up
import Profile_Importer_v3 as pim
import warnings
...
figpath = (
    "C:\\Users\\j2cle\\Work Docs\\Data\\Analysis\\SIMS\\Fig_fits"
    + "\\"
    + dt.now().strftime("%Y%m%d")
)
filepath = (
    "C:\\Users\\j2cle\\Work Docs\\Data\\Analysis\\SIMS\\Files"
    + "\\"
    + dt.now().strftime("%Y%m%d")
)
picklepath = (
    "C:\\Users\\j2cle\\Work Docs\\Data\\Analysis\\SIMS\\Pickles"
    + "\\"
    + dt.now().strftime("%Y%m%d")
)
...
imp = pim.BulkImport(["Na_2+", "E-Pixel"], calc=True) #["Na_2+", "Na+"] ["Na_2+", "Na+"] ["Na_2+", "Na+"]
imp.to_data(set_sigma=1, num_bins=5, surf_lyr=0.2)  # devs=np.nan,
imp.to_obj(set_sigma=1, num_bins=5, surf_lyr=0.2, save=True)  #devs=np.nan,
...
jar = gf.PickleJar(folder="Fit_and_Error")
redo = True
for key in imp.datas:
# for key, val in imp.datas.items():
    if (
        redo
        or not jar.database.isin([f"{gf.slugify(key)}_plot"]).any()
    ):
        ...
        prime = sfc.MatrixOps(
            sims,
            "FitProfile",
            xrange=["depth", 0, up.Length(x_lim, "um").cm, "lin"],
            yrange=["depth", 0, up.Length(x_lim, "um").cm, "lin"],
            size=75,
            min_range=r_lim,
...
for check in imp.datas:
    ...
    try:
        ...
        gf.save(data_dict, filepath, check + "_res init")
    except Exception:
        pass
