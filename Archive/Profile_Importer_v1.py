...
def gen_groups(bins, ref_arr, arr, blur=0, new=False):
    ...
    def func(loc): return xr.where((bins[loc].left < ref_arr)
                                   & (ref_arr <= bins[loc].right), 1, np.nan)
...
class ImportFunc:
    ...
    def asu_raw(self, *args):
        ...
        headers = [header_in.iloc[0, x] + ' ' + header_in.iloc[1, x] for x in range(header_in.shape[1])]
        self.data_raw = pd.read_csv(args[0], delimiter='\t', header=None, names=headers, index_col=False, skiprows=16).dropna().astype(float)
        return self.data_raw
    ...
    def rice_treated(self, *args):
        ...
        self.data_raw = pd.read_csv(args[0], delimiter='\t', header=None, names=headers, index_col=False, skiprows=5)
        return self.data_raw
    ...
    def rice_raw(self, *args):
        ...
        self.data_raw = pd.read_csv(args[0], delimiter='\s+', header=None, index_col=[0, 1, 2], names=['x', 'y', 'z', 'intens'], skiprows=10, dtype='int')
        return self.data_raw
...
class ConvFunc:
    ...
    def nrel_d(self, raw):
        self.data['Depth'] = depth_conv(self.gen_col(raw, 'na', 'x'), self.params['X unit'], self.params['Layer (actual)'], self.params['Layer (profile)'])
    ...
    def rice_raw(self, raw, **kwargs):
        ...
        for key, val in res_dict.items():
            if 'conc' in key:
                ...
                val = val.set_index(up.Length(val.index.to_numpy(), 'um').cm).reset_index()
                res_alt = {f'{samp}-{key}-{col}': val[['index', col]].rename(
                    columns={'index': 'Depth', col: 'Na'}) for col in val.columns[1:]}
                self.data = {**self.data, **res_alt}
        return self.data


class PixelConv:
    def __init__(self, raw_data, params, devs=2, surf_lyr=0.2, back_lyr=0.2, slc_strt=2, slc_stp=5, num_bins=10, set_sigma=2, set_mult=0.5, save=True, **kwargs):
        self.devs = devs
        ...
        self.outpath = 'C:\\Users\\j2cle\\Work Docs\\Data\\Analysis\\SIMS\\RICE TOF\\220425 Man'

        data_raw = ImportFunc(self.params['Reference File'], func='rice_treated').data_raw
        ...
        peak_m = xr.where((self.data_2d.grad < int(self.data_2d.grad.mean() + self.data_2d.grad.std() * self.devs)), 1, np.nan)
        peak_m = peak_m * xr.where((self.data_2d.flat < int(self.data_2d.flat.mean() + self.data_2d.flat.std() * self.devs)), 1, np.nan)
        ...
        bkg = z_eval.to_xarray().sum_intens.where(z_eval.to_xarray().sum_chng > 0, drop=True).isel(z=1)

        bins = {}
        flt_msk = self.data_2d.surf * peak_m

        bins = pd.cut(np.unique(flt_msk), self.num_bins).unique().dropna()
        def func(val): return gen_groups(bins, flt_msk, flt_msk, val)
        ...
        res_dict['proj_c'] = (self.data_1d.raw_conc.sel(z=slice(None, self.surf_lyr, None)).mean(['z']).to_dataframe().unstack('sigma').droplevel(0, 1))
        res_dict['proj_c_alt'] = (self.data_1d.corr_conc.sel(z=slice(None, self.surf_lyr, None)).mean(['z']).to_dataframe().unstack('sigma').droplevel(0, 1))
        self.res_dict = {**res_dict, **{f'{key}_s{sig}': self.data_1d[key].sel(sigma=sig).to_dataframe().unstack(['grp']).droplevel(0, 1) for key in self.data_1d.data_vars for sig in self.data_1d.sigma.to_numpy()}}
    ...
    def data_2d(self, value):
        ...
        self._data_2d['surf'] = self._matrix_ds.intens.sel(z=slice(None, self.surf_lyr, None)).sum(['z'])
        self._data_2d['back'] = self._matrix_ds.intens.sel(z=slice(self.back_lyr, None, None)).sum(['z'])
        self._data_2d['sliced'] = self._matrix_ds.intens.sel(z=slice(self.slc_strt, self.slc_stp)).mean(['z'])
        self._data_2d['grad'] = (['x', 'y'], gaussian_gradient_magnitude(self.matrix_ds.intens.sum(['z']), 1))
    ...
    def save(self):
        ...
        maps = {f'{key}_s{sig}': self.data_3d[key].sel(sigma=sig).sum(['z', 'grp']).to_dataframe()
                .unstack('x').droplevel(0, 1) for key in self.data_3d.data_vars
                for sig in self.data_3d.sigma.to_numpy()}
        maps['full1'] = (self.matrix_ds * self.sf).sum(['z']).intens.to_dataframe().unstack('x')
        maps['full2'] = (self.matrix_ds * self.sf * self.z_eval['mult'].to_xarray()).sum(['z']).intens.to_dataframe().unstack('x')
        maps['mean1'] = (self.matrix_ds * self.sf).mean(['z']).intens.to_dataframe().unstack('x')
        maps['mean2'] = (self.matrix_ds * self.sf * self.z_eval['mult'].to_xarray()).mean(['z']).intens.to_dataframe().unstack('x')
        maps['surf1'] = (self.matrix_ds * self.sf).sel(z=slice(None, self.surf_lyr, None)).sum(['z']).intens.to_dataframe().unstack('x')
        maps['surf2'] = (self.matrix_ds * self.sf * self.z_eval['mult'].to_xarray()).sel(z=slice(None, self.surf_lyr, None)).sum(['z']).intens.to_dataframe().unstack('x')

        gf.save(maps, self.outpath, f'{self.sample}_maps')

        maps1 = {f's{sig}_g{group}': self.data_3d.raw_conc.sel(grp=group, sigma=sig).sum(['z']).to_dataframe().unstack('x').droplevel(0, 1) for sig in self.data_3d.sigma.to_numpy() for group in self.data_3d.grp.to_numpy()}

        maps2 = {f's{sig}_g{group}': self.data_3d.corr_conc.sel(grp=group, sigma=sig).sum(['z']).to_dataframe().unstack('x').droplevel(0, 1) for sig in self.data_3d.sigma.to_numpy() for group in self.data_3d.grp.to_numpy()}
    ...
    def plots(self, grp_plot=None, map_plot=None, plt_type=None, surf_plt=None, prof_plt=None):
        ...
        if map_plot is not None:
            if plt_type is None:
                plt_type = 'raw_intens'
            if 'conc' in plt_type:
                (np.log10(self.data_3d[plt_type].sel(z=slice(None, self.surf_lyr, None), sigma=map_plot).sum(['z'])).plot(x='x', y='y', col='grp', col_wrap=int(self.num_bins/3)))
            else:
                (self.data_3d[plt_type].sel(z=slice(None, self.surf_lyr, None), sigma=map_plot).sum(['z']).plot(x='x', y='y', col='grp', col_wrap=int(self.num_bins/3)))
...
class BulkImport:
    def __init__(self, call, **kwargs):
        ...
        files = pd.Series([f'{prime_path}{x[0]}\\{x[1]}\\Files{x[2]}\\{x[3]}' for x in self.df_log[['Source', 'Folder', 'Sub Folder', 'File']].to_numpy()], index=self.df_log.index)

        for logs in folders.unique():
            self.params_df = pd.read_excel(f'{prime_path}{logs}\Sample Log.xlsx', index_col=0,  skiprows=1).dropna(axis=0, how='all')

        self.raws = {sample: ImportFunc(files[sample], self.df_log.loc[sample, 'Tab'], self.df_log.loc[sample, 'Columns'], func=self.df_log['Import Info'][sample]).data_raw for sample in self.df_log.index}

        self.datas = {}
        for sample in self.df_log.index:
            data = ConvFunc(self.raws[sample], self.params[sample], func=self.df_log['Import Info'][sample], **kwargs).data
...
class DataProfile:
    ...
    def __init__(self, slog, data, limit=False, loc=None, even=False, **kwargs):
        ...
        if 'Fit depth/limit' in self.params.keys() and not np.isnan(self.params['Fit depth/limit']):
            ...
            self.params['Fit depth/limit'] = lin_test(self.data['Depth'].to_numpy(), self.data['Na'].to_numpy(), 0.05)[1]
        ...
        if even:
            self.data['Depth'] = np.linspace(self.data['Depth'].min(), self.data['Depth'].max(),
                                             len(self.data['Depth']))
